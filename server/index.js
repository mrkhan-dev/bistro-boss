...
async function run() {
  try {
    ...
    const meuCollection = client.db("BistoBoss").collection("menu");
    const reviewCollection = client.db("BistoBoss").collection("reviews");

    app.get("/menu", async (req, res) => {
      const result = await meuCollection.find().toArray();
      res.send(result);
    });
